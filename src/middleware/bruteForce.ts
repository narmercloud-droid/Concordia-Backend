...
export async function isBlockedByIp(ip: string) {
  try {
...
export async function incrFailureForIp(ip: string) {
  try {
    if (!client || typeof client.incr !== "function") return;
    const v = await client.incr(ipKey(ip));
    if (v === 1) await client.expire(ipKey(ip), WINDOW_SECONDS);
    return v;
  } catch (err) {
...
export async function resetFailuresForIp(ip: string) {
  try {
...
export async function isBlockedByUser(userIdentifier: string) {
  try {
    if (!client || typeof client.get !== "function") return false;
...
export async function incrFailureForUser(userIdentifier: string) {
  try {
    if (!client || typeof client.incr !== "function") return;
    const v = await client.incr(userKey(userIdentifier));
    if (v === 1) await client.expire(userKey(userIdentifier), WINDOW_SECONDS);
    return v;
  } catch (err) {
...
export async function resetFailuresForUser(userIdentifier: string) {
  try {
    if (!client || typeof client.del !== "function") return;
