...
import {
  getAggregatedPlatformSettings,
  getBranchSettingsDetail,
  updateBranchSettingsDetail,
  updateGlobalNotificationSettings,
  updateLoyaltySettings,
  updatePlatformConfig
} from "../../services/platform/platformSettings.service.ts";
...
export const updatePlatformSettings = wrap(async (req: Request) => {
  const body = req.body ?? {};
  ...
  if (body.permissions) {
    const permissions = await updateManagerPermissionPolicy(body.permissions, (req as any).user?.id);
...
export const getBranchSettings = wrap(async (req: Request) => {
  const branchId = req.params.branchId;
...
export const updateBranchSettings = wrap(async (req: Request) => {
  const branchId = req.params.branchId;
  if (!branchId) throw fail("INVALID_INPUT", "branchId is required");
  try {
