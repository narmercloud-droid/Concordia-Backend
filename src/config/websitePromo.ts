import { getPlatformConfig } from "../services/platform/platformSettings.service.ts";
...
export function calcWebsiteDiscount(
  subtotal: number,
  branchPromotions?: Record<string, unknown> | null
) {
  if (subtotal <= 0) return 0;
  if (branchPromotions?.websiteDiscountEnabled === false) return 0;
...
export function isFreeDrinkPromoActive(
  ...
  if (!isFreeDrinkCheckoutGloballyEnabled()) return false;
