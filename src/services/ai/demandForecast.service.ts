...
export class DemandForecastService {
   ...
   */
  static async recordBranchDemand(order: any): Promise<void> {
   ...
   */
  static async predictDemand(branchId: string, date: Date, hour: number): Promise<number> {
   ...
   */
  static async get24HourForecast(branchId: string) {
    const now = new Date();
    const forecast = [];
   ...
   */
  static async getCurrentDemand(branchId: string) {
    const now = new Date();
