...
export class CourierAnalyticsService {
   ...
   */
  static async updateCourierPerformance(order: any): Promise<void> {
   ...
   */
  static async calculateCourierScore(courierId: string): Promise<number> {
   ...
   */
  static async getCourierPerformance(courierId: string) {
   ...
   */
  static async getTopCouriers(branchId: string, limit: number = 10) {
    ...
    const couriersWithScores = await Promise.all(
