...
export class DecisionEngineService {
  ...
  async pricing(branchId: string) {
    ...
    const demandCountByItemId = new Map<string, number>(
      demandCounts.map(g => [g.itemId, g._count.itemId])
    );

    // timeFactor is identical for all items in this run
    const hour = new Date().getHours();
    const timeMultiplier =
      hour >= 18 && hour <= 21 ? 1.10 : hour >= 14 && hour <= 16 ? 0.95 : 1.00;

    const results: Array<{ itemId: string; oldPrice: number; newPrice: number }> = [];
  ...
  async inventory(branchId: string) {
    ...
    for (const f of forecast as Array<{ expectedDaysUntilOut: string; name: string }>) {
  ...
  async staffing(branchId: string) {
    ...
    for (const a of alerts) {
      await this.log(
        branchId,
        "staffing",
        `High courier demand expected at hour ${a.hour}`,
        0.85,
        "high",
        false
      );
  ...
  async retention(branchId: string) {
    ...
    const results: Array<{ customerId: string; churn: { score: number; level: string } }> = [];
