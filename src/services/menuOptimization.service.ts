...
export class MenuOptimizationService {
  ...
  async underperforming(branchId: string): Promise<any[]> {
  ...
  async highPerforming(branchId: string): Promise<any[]> {
    // Get items that have been ordered from this branch
    const orderItems = await prisma.orderItem.findMany({
      where: {
        order: {
    ...
    const itemStats = new Map<string, { id: string; name: string; orderCount: number }>();
  ...
  async priceOptimization(branchId: string): Promise<any[]> {
    // Get items that have been ordered from this branch
    const orderItems = await prisma.orderItem.findMany({
      where: {
        order: {
  ...
  async searchConversion(branchId: string): Promise<any[]> {
  ...
  async inventoryInsights(branchId: string): Promise<any[]> {
    const items = await prisma.menuItem.findMany();
  ...
  async optimize(branchId: string): Promise<any> {
    return {
