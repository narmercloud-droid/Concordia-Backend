...
export const CRAWL_MENU_SLUGS = ["straelen", "kempen"] as const;
export type CrawlMenuSlug = (typeof CRAWL_MENU_SLUGS)[number];

const LOCATIONS: Record<
  CrawlMenuSlug,
  { branchId: string; label: string; servesCuisine: string[] }
> = {
  straelen: {
...
export function resolveCrawlMenuSlug(slug: string): CrawlMenuSlug | null {
  if (slug in LOCATIONS) return slug as CrawlMenuSlug;
  return null;
}
...
function buildJsonLd(input: {
  ...
  urls: ReturnType<typeof branchUrls>;
}) {
  ...
  if (opening.length) restaurant.openingHoursSpecification = opening;

  return restaurant;
}

function renderCategoryHtml(cat: MenuCategory): string {
...
export async function renderCrawlMenuHtml(slug: CrawlMenuSlug): Promise<string | null> {
  const location = LOCATIONS[slug];
  if (!isCustomerBranchVisible(location.branchId)) return null;
  ...
  const categories = (await getBranchMenuForCustomer(location.branchId, "de")) as MenuCategory[];
...
export function getCrawlMenuSitemapUrls(): string[] {
