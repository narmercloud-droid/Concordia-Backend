...
export async function sendOrderReviewInvitation(orderId: string) {
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    include: { review: true, trackingEvents: true }
  });

  if (!order || order.review) return { sent: false, reason: "not_eligible" };
  if (!shouldSendReviewInvitation(order)) return { sent: false, reason: "status" };

  const alreadyInvited = order.trackingEvents.some(
    (event) => event.status === REVIEW_INVITED_STATUS
  );
  if (alreadyInvited) return { sent: false, reason: "already_sent" };
  ...
  if (order.customerEmail) {
    tasks.push(
      sendEmail(
        order.customerEmail,
        `Concordia — Bewerten Sie Ihre Bestellung #${orderShortId}`,
  ...
  if (order.pushToken) {
    tasks.push(
      sendPush(order.pushToken, pushTitle, pushBody, {
        orderId: order.id,
        url: reviewUrl
      }).then(() => true)
    );
