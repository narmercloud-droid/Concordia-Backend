...
import { buildOrderTrackingUrl } from "../../utils/customerOrderUrls.ts";
import {
  formatEuro,
  legalPageLinksHtml,
  orderFromEmail
} from "../../utils/legalEmail.util.ts";

const RESEND_API_KEY = process.env.RESEND_API_KEY;

function fulfillmentLabel(type?: string | null) {
  if (type === "pickup") return "Abholung";
  if (type === "delivery") return "Lieferung";
  return type ?? "Bestellung";
}

function paymentLabel(method?: string | null) {
  const value = (method ?? "").toUpperCase();
  if (value === "COD") return "Barzahlung bei Lieferung/Abholung";
  if (value === "CARD") return "Karte (Stripe)";
  if (value === "PAYPAL") return "PayPal";
  if (value === "KLARNA") return "Klarna";
  if (value === "SEPA") return "SEPA-Lastschrift";
  return method ?? "—";
}
...
export async function sendOrderConfirmationEmail(orderId: string) {
  ...
  if (order.confirmationEmailSentAt) return { sent: false, reason: "already_sent" as const };
  ...
  const sent = await sendEmail(
    email,
    `Bestellbestätigung ${order.id.slice(0, 8).toUpperCase()} – Concordia Pizza`,
    html
  );
...
export async function sendGiftCardConfirmationEmail(purchaseId: string) {
  ...
  const sent = await sendEmail(
    email,
    `Gutscheinbestätigung – Concordia Pizza`,
    html
  );
