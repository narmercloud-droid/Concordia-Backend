...
import { sendGiftCardConfirmationEmail } from "./orderConfirmationEmail.service.ts";
...
function branchCodePrefix(branchId: string) {
  const slug = branchId.replace(/^concordia-/, "").replace(/[^a-z0-9]/gi, "");
  return slug.slice(0, 6).toUpperCase() || "GIFT";
}

export function generateGiftCardCode(branchId: string) {
...
export async function createGiftCardPurchase(input: GiftCardPurchaseInput) {
...
export async function validateGiftCard(
  ...
  if (!normalized) throw new Error("Bitte Gutscheincode eingeben");

  const card = await prisma.branchGiftCard.findFirst({
    where: { code: { equals: normalized, mode: "insensitive" } }
  });

  if (!card) throw new Error("Gutscheincode ungültig");
  ...
  if (balance <= 0) throw new Error("Gutschein ist aufgebraucht");
...
export async function redeemGiftCard(giftCardId: string, amount: number) {
  const card = await prisma.branchGiftCard.findUnique({ where: { id: giftCardId } });
  if (!card) throw new Error("Gutschein nicht gefunden");

  const balance = Number(card.balance);
  if (amount > balance) throw new Error("Gutscheinsaldo reicht nicht aus");
...
export async function getGiftCardPurchase(purchaseId: string) {
