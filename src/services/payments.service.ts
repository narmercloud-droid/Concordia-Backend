...
export class PaymentsService {
  ...
  async createStripePaymentIntent(orderId: string, amount: number, currency: string = "eur"): Promise<any> {
  ...
  // PAYPAL ORDER
  async createPayPalOrder(orderId: string, amount: number, currency: string = "EUR"): Promise<any> {
  ...
  async refund(orderId: string): Promise<any> {
    ...
    if (order?.paypalCaptureId) {
      const token = await getPayPalAccessToken();
      await axios.post(
        `${PAYPAL_BASE}/v2/payments/captures/${order.paypalCaptureId}/refund`,
        {},
        {
