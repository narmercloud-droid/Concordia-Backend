...
export class KnowledgeGraphService {
  ...
  async buildNodes(branchId: string) {
  ...
  > {
    ...
    const queryToItemIds = new Map<string, string[]>();
    ...
    for (const s of searchLogs) {
      ...
      if (matched.length > 0) queryToItemIds.set(q, matched);
    }

    // Precompute orderItem counts for all matched itemIds for this branch in one query
    const allMatchedItemIds = Array.from(
      new Set(Array.from(queryToItemIds.values()).flat())
    );
    ...
    const orderCountByItemId = new Map<string, number>(
      orderItemCounts.map(g => [g.itemId, g._count.itemId])
    );
