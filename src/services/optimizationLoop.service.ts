...
export class OptimizationLoopService {
  ...
  async optimizeForecasting(branchId: string): Promise<{ old: number; new: number }> {
    try {
      const params = await prisma.optimizationParameters.findUnique({ where: { branchId } });
      if (!params) throw new Error("Missing optimization parameters");
      ...
      const predicted = forecast.reduce((a: number, b: { expectedOrders: number }) => a + b.expectedOrders, 0);
      const error = Math.abs(predicted - actual) / (actual || 1);

      let newWeight = params.forecastWeight;

      if (error > 0.5) newWeight *= 0.9;
      if (error < 0.2) newWeight *= 1.1;
      ...
      if (newWeight !== params.forecastWeight) {
        await this.log(
          ...
          "forecasting",
          "forecastWeight",
          params.forecastWeight,
          newWeight,
          "Adjusted based on accuracy"
        );
  ...
  async optimizeChurn(branchId: string): Promise<{ old: number; new: number }> {
    try {
      const params = await prisma.optimizationParameters.findUnique({ where: { branchId } });
      if (!params) throw new Error("Missing optimization parameters");
      ...
      const actualOrderCountByCustomerId = new Map<string, number>(
        actualOrderCounts.map(g => [g.customerId as string, g._count.customerId])
      );
      ...
      for (const customerId of customerIds) {
        ...
        const actuallyChurned =
          (actualOrderCountByCustomerId.get(customerId) ?? 0) === 0;
      ...
      if (accuracy < 0.6) newThreshold -= 0.05;
      if (accuracy > 0.8) newThreshold += 0.05;
  ...
  async optimizeBehavior(branchId: string): Promise<{ old: number; new: number }> {
    try {
      const params = await prisma.optimizationParameters.findUnique({ where: { branchId } });
      if (!params) throw new Error("Missing optimization parameters");
      ...
      for (const c of customers) {
        ...
        if (!lastOrderItem) continue;

        if (lastOrderItem.itemId === prediction.itemId) correct++;
        total++;
      }
      ...
      if (accuracy < 0.5) newConfidence -= 0.05;
      if (accuracy > 0.8) newConfidence += 0.05;
  ...
  async run(branchId: string): Promise<{
    forecasting: { old: number; new: number };
    churn: { old: number; new: number };
    behavior: { old: number; new: number };
  }> {
    try {
      return {
