...
import { z } from "zod";
import { getChurnPrediction, cacheChurnPrediction, getTopMenuItems as getCachedTopMenu, cacheTopMenuItems, getBranchDemandForecast, cacheBranchDemandForecast } from "../../lib/redis.js";
// Validation schemas
...
export const AnalyticsController = {
     ...
     */
    async getCustomerChurnRisk(req, res, next) {
        try {
            ...
            await cacheChurnPrediction(customerId, churnRisk, analytics.totalOrders, analytics.totalSpend, analytics.lastOrderDate || null, 30);
            return success(res, {
                customerId,
     ...
     */
    async getCustomerLTV(req, res, next) {
        try {
            const parsed = idParamSchema.safeParse(req.params);
            if (!parsed.success) {
                return fail(res, "VALIDATION_ERROR", parsed.error.issues[0].message, 400);
     ...
     * Get top menu items
     */
    async getTopMenuItems(req, res, next) {
        try {
     ...
     */
    async getCourierPerformance(req, res, next) {
        try {
            const parsed = idParamSchema.safeParse(req.params);
            if (!parsed.success) {
                return fail(res, "VALIDATION_ERROR", parsed.error.issues[0].message, 400);
     ...
     */
    async getBranchDemand(req, res, next) {
        try {
            const parsed = idParamSchema.safeParse(req.params);
            if (!parsed.success) {
                return fail(res, "VALIDATION_ERROR", parsed.error.issues[0].message, 400);
