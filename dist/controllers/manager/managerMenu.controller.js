...
import * as menuCrud from "../../services/manager/managerMenuCrud.service.js";
import { storeMenuItemImage } from "../../services/upload/menuImageStorage.service.js";
...
async function requireStructureEdit(req) {
    ...
    if (!allowed)
        throw fail("FORBIDDEN", "Cannot edit menu structure");
}
export const getMenuItemDetail = wrap(async (req) => {
    const menuItemId = Number(req.params.menuItemId);
    if (!Number.isFinite(menuItemId))
        throw fail("INVALID_INPUT", "Invalid menu item id");
    try {
...
export const createCategory = wrap(async (req) => {
    await requireStructureEdit(req);
    const name = String(req.body?.name ?? "").trim();
    if (!name)
        throw fail("INVALID_INPUT", "Category name is required");
    try {
...
export const updateCategory = wrap(async (req) => {
    await requireStructureEdit(req);
    const categoryId = Number(req.params.id);
    if (!Number.isFinite(categoryId))
        throw fail("INVALID_INPUT", "Invalid category id");
    try {
...
export const deleteCategory = wrap(async (req) => {
    await requireStructureEdit(req);
    const categoryId = Number(req.params.id);
    if (!Number.isFinite(categoryId))
        throw fail("INVALID_INPUT", "Invalid category id");
...
export const createMenuItem = wrap(async (req) => {
    await requireStructureEdit(req);
    const { name, categoryId, price } = req.body ?? {};
    if (!name?.trim())
        throw fail("INVALID_INPUT", "Item name is required");
    if (!Number.isFinite(Number(categoryId)))
        throw fail("INVALID_INPUT", "categoryId is required");
    if (!Number.isFinite(Number(price)))
        throw fail("INVALID_INPUT", "price is required");
    try {
...
export const updateMenuItemFull = wrap(async (req) => {
    await requireStructureEdit(req);
...
export const uploadMenuItemImage = wrap(async (req) => {
    await requireStructureEdit(req);
    const branchMenuItemId = Number(req.params.branchMenuItemId);
    if (!Number.isFinite(branchMenuItemId))
        throw fail("INVALID_INPUT", "Invalid item id");
    const file = req.file;
    if (!file)
        throw fail("INVALID_INPUT", "Image file is required");
...
export const clearMenuItemImage = wrap(async (req) => {
    await requireStructureEdit(req);
    const branchMenuItemId = Number(req.params.branchMenuItemId);
    if (!Number.isFinite(branchMenuItemId))
        throw fail("INVALID_INPUT", "Invalid item id");
...
export const deleteMenuItem = wrap(async (req) => {
    await requireStructureEdit(req);
    const branchMenuItemId = Number(req.params.branchMenuItemId);
    if (!Number.isFinite(branchMenuItemId))
        throw fail("INVALID_INPUT", "Invalid item id");
...
export const createVariantGroupHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const menuItemId = Number(req.params.menuItemId);
    if (!Number.isFinite(menuItemId))
        throw fail("INVALID_INPUT", "Invalid menu item id");
    if (!req.body?.name?.trim())
        throw fail("INVALID_INPUT", "Group name is required");
    try {
...
export const updateVariantGroupFull = wrap(async (req) => {
    await requireStructureEdit(req);
...
export const deleteVariantGroupHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const groupId = req.params.groupId;
    if (!groupId)
        throw fail("INVALID_INPUT", "group id is required");
...
export const createVariantHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const groupId = req.params.groupId;
    if (!groupId || !req.body?.name?.trim())
...
export const updateVariantHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const variantId = req.params.variantId;
    if (!variantId)
        throw fail("INVALID_INPUT", "variant id is required");
    try {
...
export const deleteVariantHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const variantId = req.params.variantId;
    if (!variantId)
        throw fail("INVALID_INPUT", "variant id is required");
...
export const createAddOnGroupHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const menuItemId = Number(req.params.menuItemId);
    if (!Number.isFinite(menuItemId))
        throw fail("INVALID_INPUT", "Invalid menu item id");
...
export const updateAddOnGroupHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const groupId = req.params.groupId;
    if (!groupId)
        throw fail("INVALID_INPUT", "group id is required");
...
export const deleteAddOnGroupHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const groupId = req.params.groupId;
    if (!groupId)
        throw fail("INVALID_INPUT", "group id is required");
...
export const createAddOnHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const groupId = req.params.groupId;
    if (!groupId || !req.body?.name?.trim())
        throw fail("INVALID_INPUT", "name is required");
...
export const updateAddOnHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const addOnId = req.params.addOnId;
    if (!addOnId)
        throw fail("INVALID_INPUT", "add-on id is required");
    try {
...
export const deleteAddOnHandler = wrap(async (req) => {
    await requireStructureEdit(req);
    const addOnId = req.params.addOnId;
    if (!addOnId)
        throw fail("INVALID_INPUT", "add-on id is required");
