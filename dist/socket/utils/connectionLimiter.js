// ===== GLOBAL STATE =====
const ipConnections = new Map(); // IP -> socket IDs
const namespaceConnections = new Map(); // namespace -> connection state
 ...
 */
export const canConnect = (socket, config = DEFAULT_CONFIG) => {
 ...
 */
export const registerConnection = (socket) => {
    const clientIP = socket.handshake.address;
    const namespace = socket.nsp.name;
    if (!ipConnections.has(clientIP)) {
        ipConnections.set(clientIP, new Set());
 ...
 */
export const unregisterConnection = (socket) => {
    const clientIP = socket.handshake.address;
    const namespace = socket.nsp.name;
 ...
 */
export const getIPConnectionCount = (clientIP) => {
    return ipConnections.get(clientIP)?.size || 0;
};
/**
 * Get connection count for a namespace
 */
export const getNamespaceConnectionCount = (namespace) => {
 ...
 */
export const disconnectIP = (socket, clientIP) => {
 ...
 */
export const disconnectNamespace = (namespace, _reason = "Server reset") => {
    if (!namespaceConnections.has(namespace))
        return;
    const nsState = namespaceConnections.get(namespace);
 ...
 */
export const createConnectionLimitMiddleware = (config = DEFAULT_CONFIG) => {
    return (socket, next) => {
