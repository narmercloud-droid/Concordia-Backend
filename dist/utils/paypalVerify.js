...
export async function verifyPayPalWebhook(req, webhookId) {
...
export async function verifyPayPalWebhookWithAnyId(req, webhookIds) {
    const unique = [...new Set(webhookIds.filter(Boolean))];
    for (const webhookId of unique) {
        if (await verifyPayPalWebhook(req, webhookId))
            return true;
    }
    return false;
}
