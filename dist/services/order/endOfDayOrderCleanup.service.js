...
import { advanceTerminalOrderStatus } from "../terminal/terminalOrderStatus.service.js";
import { IN_PROGRESS_ORDER_STATUSES, TERMINAL_DONE_ORDER_STATUSES, UNPAID_ONLINE_PAYMENT_STATUSES } from "./orderStatus.constants.js";
...
function shouldCancelStaleOrder(order) {
    if (UNPAID_ONLINE_PAYMENT_STATUSES.includes(order.paymentStatus)) {
        return true;
    }
    ...
    for (const order of staleOrders) {
        try {
            if (shouldCancelStaleOrder(order)) {
                ...
                cancelled += 1;
                logger.info({ orderId: order.id, branchId: order.branchId, status: order.status }, "Cancelled stale unpaid/unaccepted order at end of day");
                continue;
            }
            const targetStatus = isPickupOrder(order) ? "picked_up" : "delivered";
            await advanceTerminalOrderStatus(order.id, targetStatus);
            closed += 1;
            logger.info({ orderId: order.id, branchId: order.branchId, targetStatus }, "Auto-completed stale order at end of day");
        }
        catch (err) {
            skipped += 1;
            logger.warn({ orderId: order.id, branchId: order.branchId, err }, "Failed end-of-day order cleanup for order");
        }
    }
    logger.info({ berlinDate: ymd, closed, cancelled, skipped, total: staleOrders.length }, "End-of-day order cleanup finished");
    return { closed, cancelled, skipped };
}
export function isTerminalDoneStatus(status) {
    return TERMINAL_DONE_ORDER_STATUSES.includes(status);
}
