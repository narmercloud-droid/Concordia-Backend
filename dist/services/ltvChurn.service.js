...
export class LtvChurnService {
    ...
    async predictLtv(customerId, branchId) {
    ...
    async churnRisk(customerId, branchId) {
    ...
    async segment(customerId, branchId) {
        const ltv = await this.predictLtv(customerId, branchId);
        const churn = await this.churnRisk(customerId, branchId);
        let segment = "low";
        if (ltv.predictedLtv > 500)
            segment = "VIP";
        else if (ltv.predictedLtv > 200)
            segment = "high";
        else if (ltv.predictedLtv > 50)
            segment = "medium";
        else
            segment = "low";
        if (churn.level === "high")
