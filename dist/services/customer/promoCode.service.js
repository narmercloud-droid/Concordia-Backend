...
function calcDiscountAmount(type, amount, orderTotal) {
...
export async function validatePromoCode(code, orderTotal) {
    ...
    if (!promo.isActive)
        throw new Error("Gutscheincode ist nicht mehr gültig");
    ...
    if (orderTotal < promo.minOrder) {
        throw new Error(`Mindestbestellwert ${promo.minOrder.toFixed(2).replace(".", ",")} € für diesen Gutschein`);
    }
...
export async function redeemPromoCode(promoCodeId) {
