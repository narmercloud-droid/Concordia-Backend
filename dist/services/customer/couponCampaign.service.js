...
function campaignScope(branchId) {
    return branchId ? "branch" : "platform";
}
function isWithinDates(validFrom, validUntil) {
    const now = Date.now();
...
export function calcCampaignDiscount(discountType, discountValue, orderTotal) {
...
export async function listActiveCampaignsForBranch(branchId, customerId) {
    ...
    const visible = rows.filter((row) => isWithinDates(row.validFrom, row.validUntil) &&
...
export async function createCampaign(branchId, input) {
...
export async function updateCampaign(campaignId, branchId, input) {
