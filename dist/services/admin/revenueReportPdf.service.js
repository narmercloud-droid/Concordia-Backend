...
import { getRevenueReport } from "./revenueReport.service.js";
...
export async function buildRevenueReportPdf(report) {
    return new Promise((resolve, reject) => {
        ...
        doc.moveDown(0.5);
        doc.fontSize(10);
        doc.text(`Gastbestellungen: ${report.customerTypeBreakdown.guest.count} — ${formatEur(report.customerTypeBreakdown.guest.total)}`);
        doc.text(`Registrierte Kunden: ${report.customerTypeBreakdown.registered.count} — ${formatEur(report.customerTypeBreakdown.registered.total)}`);
        doc.moveDown(1);
        doc.fontSize(13).text("Neu vs. Stammkunden (Filiale)", { underline: true });
        doc.moveDown(0.5);
        doc.fontSize(10);
        doc.text(`Erstbesteller: ${report.newReturningBreakdown.newCustomers.count} — ${formatEur(report.newReturningBreakdown.newCustomers.total)}`);
        doc.text(`Wiederkehrend: ${report.newReturningBreakdown.returningCustomers.count} — ${formatEur(report.newReturningBreakdown.returningCustomers.total)}`);
        if (report.newReturningBreakdown.unknown.count > 0) {
            doc.text(`Ohne Zuordnung: ${report.newReturningBreakdown.unknown.count} — ${formatEur(report.newReturningBreakdown.unknown.total)}`);
        }
        doc.moveDown(1.5);
        doc.fontSize(9).fillColor("#555").text(report.legalNote, { align: "justify" });
        doc.moveDown(0.5);
        doc.text("Hinweis: Dieses Dokument ersetzt keine steuerliche Beratung. Eine detaillierte MwSt.-Aufteilung (7 % / 19 %) ist bei Bedarf über den Steuerberater zu erstellen.", { align: "justify" });
        doc.text("Concordia Bestellsystem — automatisch generiert.", { align: "center" });
        doc.end();
    });
}
export async function getRevenueReportPdf(params) {
