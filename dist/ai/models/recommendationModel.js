...
export class RecommendationModel {
     ...
     */
    static async recommendItems(customerId, branchId, limit = 5) {
        try {
     ...
     */
    static async getComplementaryItems(itemId, branchId, limit = 3) {
        try {
            ...
            const topCoOccurring = Object.entries(coOccurrenceCounts)
                ...
                .slice(0, limit);
            // Fetch item details
            const complementaryItems = await Promise.all(topCoOccurring.map(async ([itemId, count]) => {
