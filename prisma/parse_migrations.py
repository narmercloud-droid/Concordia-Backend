        ...
        if s.upper().startswith('CREATE TABLE'):
            ...
            for line in body.splitlines():
                ...
                if line.upper().startswith('CONSTRAINT'):
                    if 'PRIMARY KEY' in line.upper():
                        ...
                        if p: pk = [c.strip().strip('"') for c in p.group(1).split(',')]
                    continue
                cols.append(line)
            ...
            for col in cols:
                if col.startswith('"'):
                    ...
                    colname=parts[0].strip('"'); coltype=parts[1]; raw=col
                    state[name]['columns'][colname]={'type':coltype,'raw':raw,'notnull':'NOT NULL' in col.upper(),'default':None,'unique':False,'autoincrement':'SERIAL' in coltype.upper()}
                    if 'DEFAULT' in col.upper():
                        import re
                        d = re.search(r'DEFAULT (.+)', col, re.I)
                        if d: state[name]['columns'][colname]['default']=d.group(1).strip()
            ...
            if m:
                unique=bool(m.group(1)); table=m.group(3); col=m.group(4)
            ...
            if not m: continue
            table=m.group(1); body=m.group(2).strip()
            if body.upper().startswith('ADD COLUMN'):
                rest=body[len('ADD COLUMN'):].strip()
                # if multiple add column segments separated by commas on same stmt
                parts=[]; current=''
                depth=0
                for ch in rest:
                    if ch==',' and depth==0:
                ...
                for fragment in parts:
                    ...
                    name=items[0].strip('"'); typ=items[1]; raw=fragment
                    state.setdefault(table,{'columns':{},'pk':[],'fks':[],'indexes':[]})['columns'][name]={'type':typ,'raw':raw,'notnull':'NOT NULL' in fragment.upper(),'default':None,'unique':False,'autoincrement':'SERIAL' in typ.upper()}
                    if 'DEFAULT' in fragment.upper():
                        d=re.search(r'DEFAULT ([^,]+)', fragment, re.I)
                        if d: state[table]['columns'][name]['default']=d.group(1).strip()
...
for t,idx in indexes.items():
    if t in state: state[t].setdefault('indexes',[]).extend(idx)
with open('migration_state.json','w',encoding='utf8') as f: json.dump({'tables':state,'enums':enums},f,indent=2)
